...
============================================================
  ...
  Credentials: hardcoded in CONFIGURATION section below.
============================================================
"""
...
# ══════════════════════════════════════════════════════════
#  FFMPEG LOCATION
#  yt-dlp's ffmpeg_location must be the DIRECTORY containing
#  the ffmpeg binary (e.g. /usr/bin), NOT the binary itself.
#  Detected once at module load so it's available everywhere.
# ══════════════════════════════════════════════════════════
def _detect_ffmpeg_dir() -> str:
    ...
    # Common fallback locations on Ubuntu/Debian
...
def _ytdlp_download_sync(query: str) -> dict:
    ...
    ffmpeg_location fix: yt-dlp expects the DIRECTORY containing
    ffmpeg (e.g. /usr/bin), not the binary path (/usr/bin/ffmpeg).
    Passing the binary path causes silent post-processor failure.
    """
    os.makedirs(CACHE_DIR, exist_ok=True)
    ...
    logger.info("Using ffmpeg dir: %s", FFMPEG_DIR)
    ...
    base_opts = {
        ...
        "retries": 5,
        "fragment_retries": 5,
        # DIRECTORY path — yt-dlp appends /ffmpeg to this internally
        "ffmpeg_location": FFMPEG_DIR,
        "extractor_args": {
            "youtube": {
                "player_client": ["android", "android_embedded", "android_vr"]
            }
        },
        # Spoof Android YouTube app User-Agent to avoid HTTP 403 on VPS IPs
        "http_headers": {
            "User-Agent": (
    ...
    # Step 1: info only — get video_id without downloading
    ...
    if not mp3_file or not os.path.exists(mp3_file):
        raise FileNotFoundError(
            f"yt-dlp finished but no audio file found in {CACHE_DIR}/. "
            "ffmpeg post-processor may have failed silently."
        )
...
async def cmd_play(message: Message) -> None:
    ...
    try:
        ...
        logger.error("ffmpeg/file error: %s", exc)
...
async def main() -> None:
    ...
    # 1. Validate ffmpeg at startup — fail early with clear message
    if not FFMPEG_DIR:
        logger.error("ffmpeg not found! Run: sudo apt install ffmpeg -y")
        return
    logger.info("ffmpeg directory: %s", FFMPEG_DIR)
    ...
    # 4. Create all clients inside this coroutine (same event loop)
    userbot = Client(
    ...
    logger.info(
        ...
        "   ffmpeg dir         : %s\n"
        "==========================================",
        CACHE_DIR, cached_count, FFMPEG_DIR
    )
